...
pub struct Id;
impl marker::Symmetric for Id {}
impl marker::Linear for Id {}
...
impl<M> marker::Symmetric for NeumannSeries<M> where M: marker::Symmetric {}
impl<M> marker::Linear for NeumannSeries<M> where M: marker::Linear {}
...
impl<S> marker::Symmetric for Restart<S> where S: marker::Symmetric {}
impl<S> marker::Linear for Restart<S> where S: marker::Linear {}
...
impl<S1,S2> marker::Symmetric for Concat<S1,S2> where S1: marker::Symmetric, S2: marker::Symmetric {}
impl<S1,S2> marker::Linear for Concat<S1,S2> where S1: marker::Linear, S2: marker::Linear {}
...
pub struct CG<F> where F: BreakCondition {
    max_iters: u32,
    per_iter: F,
}
impl marker::Linear for CG<NoResCeck> {}

impl<F> CG<F> where F: BreakCondition {
...
impl<V,F> SolverImpl<V> for CG<F> where V: Vector, F: BreakCondition {
    fn size(&self) -> usize {
        3
    }
...
pub struct PCG<F,P> where F: BreakCondition {
    cg: CG<F>,
    preconditioner: P,
}
impl<P> marker::Linear for PCG<NoResCeck,P> where P: marker::Linear {}

impl<F,P> PCG<F,P> where F: BreakCondition {
    pub fn new(p: P, cg: CG<F>) -> Self {
        PCG {
            cg: cg,
            preconditioner: p,
        }
...
impl<V,F,P> SolverImpl<V> for PCG<F,P> where V: Vector, F: BreakCondition, P: Preconditioner<V> {
    fn size(&self) -> usize {
    ...
    fn solve<M>(&self, A: &M, x: &mut V, b: &V, vecs: &mut [V]) where M: Matrix<V> {
        ...
        if !self.cg.per_iter.continue_iter(r.norm2_sq().sqrt()) || alpha0 == 0.0 { // for the case that per_iter(.) == true
            return;
        }
        ...
        'iteration: for k in 0..self.cg.max_iters {
            ...
            //// r -= lambda v
            r.inc_ax(-lambda, &v);
            if !self.cg.per_iter.continue_iter(r.norm2_sq().sqrt()) { // we cannot use a_k+1 for convergence checks
                break 'iteration;
            }
            //// z = P^-1 r
...
pub struct BiCGStab<F> where F: BreakCondition {
    max_iters: u32,
    per_iter: F,
}
// not Linear as there exist critical conditions

impl<F> BiCGStab<F> where F: BreakCondition {
    pub fn new(max_iters: u32, per_iter: F) -> Self {
        BiCGStab {
            max_iters: max_iters,
            per_iter: per_iter,
...
impl<V,F> SolverImpl<V> for BiCGStab<F> where V: Vector, F: BreakCondition {
    fn size(&self) -> usize {
        6
    }
    ...
    fn solve<M>(&self, A: &M, x: &mut V, b: &V, vecs: &mut [V]) where M: Matrix<V> {
        ...
        if !self.per_iter.continue_iter(r.norm2_sq().sqrt()) || rho == 0.0 { // for the case that per_iter(.) == true
            return;
        }
        ...
        'iteration: for k in 0..self.max_iters {
            ...
            if !self.per_iter.continue_iter(r.norm2_sq().sqrt()) { // TODO  || rho == 0.0 ?
                break 'iteration;
            }
            //// rho_k+1 = <r,r0>
...
pub type BiCGStabM<F> = Restart<BiCGStab<F>>;
